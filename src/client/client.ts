...
export const setBankIsOpen = (bool: boolean) => {
  ...
  isBankOpen = bool;
  SendNUIMessage({ app: 'BANK', method: 'setVisible', data: bool });
...
export const setAtmIsOpen = (bool: boolean) => {
  ...
  isAtmOpen = bool;
  SendNUIMessage({ app: 'ATM', method: 'setVisibleATM', data: bool });
  SetNuiFocus(bool, bool);
};
...
if (!useFrameworkIntegration) {
  ...
  RegisterCommand(
    ...
    () => {
      ...
      isAtmOpen = !isAtmOpen;
      SendNUIMessage({ app: 'ATM', method: 'setVisibleATM', data: isAtmOpen });
