...
const Devbar = () => {
  ...
  useEffect(() => {
    if (isBankOpen) {
      window.postMessage({ app: 'BANK', method: 'setVisible', data: true });
      window.postMessage({ app: 'ATM', method: 'setVisibleATM', data: false });
    } else {
      window.postMessage({ app: 'BANK', method: 'setVisible', data: false });
    }
  }, [isBankOpen]);
  ...
  useEffect(() => {
    if (isAtmOpen) {
      window.postMessage({ app: 'BANK', method: 'setVisible', data: false });
      window.postMessage({ app: 'ATM', method: 'setVisibleATM', data: true });
    } else {
      window.postMessage({ app: 'ATM', method: 'setVisibleATM', data: false });
    }
  }, [isAtmOpen]);
